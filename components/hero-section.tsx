...
function FeatureItem({ icon: Icon, label, isRTL }: { icon: React.ElementType; label: string; isRTL?: boolean }) {
  return (
...
export function HeroSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-background min-h-screen-safe flex flex-col items-center justify-center md:flex-row md:items-center pt-16 md:pt-0">
      {/* Spotlight Effects */}
      <div>
        <Spotlight
      ...
      <style jsx>{`
        ...
        @media (max-width: 767px) {
          .mobile-animate-image {
            animation: heroScale 0.8s ease-out forwards;
          }
          .mobile-animate-overlay {
            animation: heroOverlayReveal 0.7s ease-out forwards;
            opacity: 1;
          }
          .mobile-animate-title {
            animation: heroFadeUp 0.5s ease-out 0.2s forwards;
            opacity: 0;
          }
      ...
      <div className={`absolute inset-0 hidden md:block ${isRTL ? "scale-x-100" : "-scale-x-100"}`}>
        <Image
          src="/hero.jpg"
          alt=""
          fill
      ...
      {/* Grid Background */}
      <div className="absolute left-0 top-0 flex h-full w-full items-center justify-center bg-grid dark:bg-grid">
      ...
      {/* Hero Image - Mobile: behind everything */}
      <div className={`absolute inset-0 md:hidden mobile-animate-image ${isRTL ? "" : "-scale-x-100"}`}>
        <Image
          src="/hero.jpg"
          alt=""
          fill
          priority
          className="object-cover object-left-bottom dark:opacity-65"
        />
        {/* Dark overlay that fades away */}
        <div className="absolute inset-0 bg-background mobile-animate-overlay" />
      </div>
      {/* Mobile bottom fade for smooth transition to next section */}
      <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-background to-transparent md:hidden z-10" />

      <div className="container relative mx-auto px-4 md:px-6 flex flex-col items-center md:items-start justify-center md:h-full">
        <div className={`max-w-2xl text-center md:text-left ${isRTL ? "md:mr-0 md:ml-auto md:text-right" : "md:ml-0 md:mr-auto"} mx-auto md:mx-0`}>
          <h1 style={{ opacity: 0 }} className={`hero-title mb-3 md:mb-6 font-bold tracking-tight text-balance mobile-animate-title md:!opacity-100 ${isRTL ? "leading-relaxed" : "leading-tight"}`}>
            {t("title1")}
            <br />
          ...
          <p className="hidden md:block mb-5 md:mb-10 text-sm sm:text-base md:text-lg lg:text-xl leading-relaxed text-muted-foreground text-pretty">
            {t("description")}
          </p>

          <div style={{ opacity: 0 }} className={`flex flex-col gap-3 items-center sm:flex-row sm:gap-4 mobile-animate-button md:!opacity-100 ${isRTL ? "sm:flex-row-reverse sm:justify-end" : "sm:justify-start md:items-start"}`} role="group" aria-label="Call to action buttons">
            <Button
            ...
            >
              {isRTL && <ArrowRight className="mr-2 h-4 w-4 md:h-5 md:w-5 transition-transform rotate-180 group-hover:-translate-x-1" aria-hidden="true" />}
              {t("startProject")}
              {!isRTL && <ArrowRight className="ml-2 h-4 w-4 md:h-5 md:w-5 transition-transform group-hover:translate-x-1" aria-hidden="true" />}
              <span className={`absolute ${isRTL ? "-left-1" : "-right-1"} -top-1 h-2 w-2 bg-foreground/20 hidden md:block`} aria-hidden="true" />
            </Button>
            <Button
              onClick={() => scrollToSection("services")}
              size="lg"
              variant="outline"
              className="relative hidden md:inline-flex w-full sm:w-auto border-2 border-accent text-accent text-sm md:text-base font-semibold hover:bg-accent hover:text-white transition-all duration-200 hover:shadow-lg focus:ring-2 focus:ring-accent focus:ring-offset-2"
              aria-label={t("viewServices")}
            >
              {t("viewServices")}
              <span className={`absolute ${isRTL ? "-left-1" : "-right-1"} -top-1 h-2 w-2 bg-accent/20 hidden md:block`} aria-hidden="true" />
            </Button>
          </div>

          <div className={`hidden md:grid mt-6 md:mt-12 grid-cols-2 gap-3 justify-items-center md:justify-items-start md:flex md:flex-wrap md:gap-6 ${isRTL ? "md:flex-row-reverse md:justify-end" : "md:justify-start"}`} role="list" aria-label="Key features">
            {features.map((feature, index) => (
              <div key={feature.label} className={`flex items-center gap-4 md:gap-6 ${isRTL ? "flex-row-reverse" : ""}`}>
                <FeatureItem icon={feature.icon} label={feature.label} isRTL={isRTL} />
