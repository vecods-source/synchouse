...
// Suppress THREE.js NaN warnings (known issue with three-globe during initialization)
if (typeof window !== "undefined") {
  const originalWarn = console.warn
  const originalError = console.error
  const suppressPattern = /Computed radius is NaN|BufferGeometry|position.*NaN/i

  console.warn = function(...args: any[]) {
    const msg = args[0]?.toString?.() || String(args[0])
    if (suppressPattern.test(msg)) return
...
// Type declaration for threeGlobe JSX element
...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    if (globeRef.current && globeData && !hasAnimationStarted.current) {
